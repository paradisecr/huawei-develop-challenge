...
demo.Util = {
    ...
    registerImageByUrl: function (url) {
        var image = new Image();
        image.src = url;
        var views = arguments;
        image.onload = function () {
            twaver.Util.registerImage(demo.Util.getImageName(url), url, image.width, image.height);
            image.onload = null;
            for (var i = 1; i < views.length; i++) {
                var view = views[i];
                if (view.invalidateElementUIs) {
    ...
    setMagnifyInteraction: function (network) {
...
//            demo.Util.setMagnifyInteraction(network);
//        });
...
//                newWindow = window.open(uriContent, 'network');
//            }
//            box.clear();
...
//        });
    ...
    addButton: function (div, name, src,cssClass, callback) {
        ...
        if (src) {
            button.style.padding = '7px 7px 7px 7px';
            button.style.margin = '8px 4px 4px 8px';
...
twaver.Util.ext('demo.KPICardUI', twaver.network.GridUI, {
    drawBody: function () {
        if (!this._canvas) {
            this._canvas = twaver.Util.createCanvas();
        }
        this.addComponent(this._canvas);

        var rect = this.getBodyRect();
        var g = this.setShadow(this, this._canvas, rect);
